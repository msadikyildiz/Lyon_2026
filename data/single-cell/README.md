...
The single-cell inputs contain count matrices, Samuel Cooke’s R/Seurat analysis code, GEO metadata templates, expression tables and Excel volcano plots. `input_manifest.json` records archive paths, checksums and equivalent archive copies for 31 source files (195,521,222 bytes).
...
`scripts/reproduce_single_cell.R` executes the relevant original Rmd chunks and exports the computed object under ignored `runs/single-cell/`, plus portable CSV results under `generated/`. The reproducible analysis environment uses R 4.4.2, Seurat 5.2.1 and SeuratObject 5.0.2. The original report identifies Seurat 5 but does not record its exact minor version. The complete analysis environment is in `single-cell-renv.lock`; `generated/R-session.txt` records the executed session.
...
Both clusters 4 and 9 have a larger fraction of hipA-positive cells than the remaining clusters. Mean normalized expression is higher in cluster 9 and slightly lower in cluster 4 (log2 fold changes 0.597 and −0.136). Detection frequency and mean expression describe different aspects of these sparse distributions. The cluster comparisons are in Supplementary Table 3; culture comparisons and targeted checks are in Supplementary Table 4. Source Data include both.
...
The exported tables contain the complete source worksheets, a separate worksheet of targeted checks, and all 133 parent-up enrichment results from the matching EcoCyc export. `plot_tables/table_mapping.json` makes the numbering explicit.

The [EcoCyc enrichment record](enrichment/README.md) documents five original exports, their worksheet mappings, exact result checks, and the documented Fisher Exact/Benjamini–Hochberg workflow with P < 0.1. EcoCyc 29.0 and default annotation-specific backgrounds are assumed from the May 2025 dates and reported standard workflow; the enrichment record distinguishes these assumptions from the supplied results. GSE314756 is scheduled for public release on 21 December 2026.
