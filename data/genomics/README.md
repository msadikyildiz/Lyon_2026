...
`FolderDate` and `source_file` are metadata columns. Keep the mutation-calling/reference provenance and the final plotting notebook version with the tables. If rerunning the mutation calls is intended, reference genome, caller version/options and read accessions are also needed. The supplied notebooks cover plotting and downstream table processing, not a complete raw-read alignment/calling workflow.
