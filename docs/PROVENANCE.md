...
Genomic metadata and saved displayed tables are described in `data/genomics/README.md`. Notebook outputs and execution metadata were cleared; scientific code is retained with portable paths and a repository-root bootstrap. The original per-sample mutation-call files and complete single-cell analysis inputs remain unavailable.
